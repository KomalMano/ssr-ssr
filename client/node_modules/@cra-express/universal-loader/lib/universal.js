"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports["default"] = void 0;

var path = require('path');

var fs = require('fs');

var http = require('http'); // const stringRenderer = require('./renderer/string-renderer').default;


var pipeStreamRenderer = require('./renderer/pipe-stream-renderer')["default"];

var craServiceName = process.env.CRA_SERVICE_NAME || 'localhost';
var craClientPort = process.env.CRA_CLIENT_PORT || 3000;
var defaultHtmlFilename = 'index.html';

function resolveHtmlFilenameByRequest(req, options) {
  if (!options.resolveHtmlFilenameByRequest) {
    return defaultHtmlFilename;
  }

  return options.resolveHtmlFilenameByRequest(req) || defaultHtmlFilename;
}

function handleDevMode(req, res, options) {
  var url = "http://".concat(craServiceName, ":").concat(craClientPort, "/").concat(resolveHtmlFilenameByRequest(req, options));
  http.get(url, function (result) {
    result.setEncoding('utf8');
    var htmlData = '';
    result.on('data', function (chunk) {
      htmlData += chunk;
    });
    result.on('end', function () {
      processRequest(req, res, htmlData, options);
    });
  }).on('error', function (e) {
    console.error(e.message);
    return res.status(404).end();
  });
}

function createUniversalMiddleware(options) {
  var clientBuildPath = options.clientBuildPath;

  function universalLoader(req, res) {
    if (process.env.NODE_ENV === 'development') {
      handleDevMode(req, res, options);
      return;
    }

    var filePath = path.resolve(clientBuildPath, resolveHtmlFilenameByRequest(req, options));
    fs.readFile(filePath, 'utf8', function (err, htmlData) {
      if (err) {
        console.error('read err', err);
        return res.status(404).end();
      }

      processRequest(req, res, htmlData, options);
    });
  }

  return universalLoader;
}

function processRequest(req, res, htmlData, options) {
  var universalRender = options.universalRender,
      _options$handleRender = options.handleRender,
      handleRender = _options$handleRender === void 0 ? pipeStreamRenderer : _options$handleRender;
  var data = universalRender(req, res);

  if (data === undefined) {
    return;
  }

  if (data instanceof Promise) {
    data.then(function (resolvedData) {
      if (resolvedData === undefined) {
        return;
      }

      handleRender(req, res, resolvedData, htmlData, options);
    });
    return;
  }

  handleRender(req, res, data, htmlData, options);
}

var _default = createUniversalMiddleware;
exports["default"] = _default;